...
let frameCount = 0;
let spinHistory = JSON.parse(localStorage.getItem('roulette_history') || '[]');
function isRed(num) {
...
function saveResult(num) {
    ...
    if (display) {
        ...
        setTimeout(() => { if (display)
            display.style.color = '#fff'; }, 1000);
    }
}
...
window.cvSocket = socket;
window.triggerBoardDetected = (data) => {
...
window.simulateCVUpdate = (data) => {
    ...
    window.cvWheelRPM = cvWheelRPM;
    window.cvBallRPM = cvBallRPM;
};
socket.on('prediction_update', (data) => {
    window.cvCalibrated = data.calibrated || false;
    window.cvBallFound = data.ball_found || false;
    window.cvWheelFound = data.wheel_found || false;
    window.simulateCVUpdate(data);
});
socket.on('spin_finished', (data) => {
    if (data.number !== -1) {
...
async function selectWindow(windowData) {
    try {
...
function updateUI() {
    ...
    if (hasMotion) {
        ...
        motionStatus = window.cvCalibrated ? 'CALIBRATED & SCANNING' : 'SEARCHING FOR WHEEL...';
        motionColor = window.cvCalibrated ? '#00ff00' : '#ffff00';
    }
    telemetryPanel.innerHTML = `
        ...
        return `<div title="${res.timestamp}" style="width:22px; height:22px; background:${color}; color:white; display:flex; align-items:center; justify-content:center; border-radius:3px; font-size:10px; font-weight:bold; border:1px solid #444;">${res.number === 37 ? '00' : res.number}</div>`;
            ...
            <div style="font-size:9px; color:#888; margin-left:10px;">
                F:${frameCount} | ${window.cvCalibrated ? '✓' : '✗'} | B:${window.cvBallFound ? '✓' : '✗'} | W:${window.cvWheelFound ? '✓' : '✗'}
            </div>
        </div>
    `;
    if (boardDetected) {
